...
async def accept_requests(reader, writer, job_id=[0]):
 ...
 if op[0] == 0:
	...
	await submit_job(job_id, reader, writer) #Errors in async
	job_id[0] += 1
  elif op[0] == 1:
	await get_results(reader, writer)
def worker(): # daemon
 with TrocessPoolExecutor(max_workers=5) as executor:
 
	job_id, code, data = work_queue.get() # blocking
	func = types.FunctionType(code, globals(), 'mapper_and_reducer')
	mapper, reducer = func()
	counts = executor.map_reduce(executor, data, mapper, reducer, 100, mr.reporter)
